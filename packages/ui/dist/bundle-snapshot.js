...
function create_if_block$1(state, component) {
	...
	return {
		...
		m: function mount(target, anchor) {
			...
			appendNode(dd, dl);
			appendNode(dt, dl);
			appendNode(text_1, dt);
			appendNode(dd_1, dl);
			appendNode(dt_1, dl);
			appendNode(text_4, dt_1);
			appendNode(dd_2, dl);
			appendNode(dt_2, dl);
			appendNode(text_7, dt_2);
		},

		p: function update(changed, state) {
			if (changed.simple) {
				text_1.data = state.simple;
			}

			if (changed.complex) {
				text_4.data = state.complex;
			}

			if (changed.readable) {
				text_7.data = state.readable;
			}
		},

...
function encapsulateStyles$1(node) {
	setAttribute(node, "svelte-281654481", "");
}

function add_css$1() {
	var style = createElement("style");
	style.id = 'svelte-281654481-style';
	style.textContent = "p[svelte-281654481],[svelte-281654481] p{color:crimson\n  }";
...
function Component$2(options) {
	...
	if (!document.getElementById("svelte-281654481-style")) add_css$1();
...
function create_main_fragment$1(state, component) {
	var header, text_2, div, form, div_1, label, text_4, input, input_updating = false, text_5;
	...
	return {
		...
		m: function mount(target, anchor) {
			...
			errormessage._mount(div_1, null);
		},
...
function create_if_block$2(state, component) {
	var p, text_1, dl, dd, dt, text_3, text_4, span, dd_1, dt_1, text_8, text_9, span_1, dd_2, dt_2, text_13, text_14, span_2;

	function click_handler(event) {
		var state = component.get();
		component.copy(state.simple);
	}

	function click_handler_1(event) {
		var state = component.get();
		component.copy(state.complex);
	}

	function click_handler_2(event) {
		var state = component.get();
		component.copy(state.readable);
	}

	return {
		c: function create() {
			p = createElement("p");
			p.textContent = "Your generated passwords";
			text_1 = createText("\n    ");
			dl = createElement("dl");
			dd = createElement("dd");
			dd.textContent = "Simple:";
			dt = createElement("dt");
			text_3 = createText(state.simple);
			text_4 = createText("\n        ");
			span = createElement("span");
			span.textContent = "📋";
			dd_1 = createElement("dd");
			dd_1.textContent = "Complex:";
			dt_1 = createElement("dt");
			text_8 = createText(state.complex);
			text_9 = createText("\n        ");
			span_1 = createElement("span");
			span_1.textContent = "📋";
			dd_2 = createElement("dd");
			dd_2.textContent = "Readable:";
			dt_2 = createElement("dt");
			text_13 = createText(state.readable);
			text_14 = createText("\n        ");
		...
		h: function hydrate() {
			...
			span_1.className = "hover-hidden";
			addListener(span_1, "click", click_handler_1);
			span_2.className = "hover-hidden";
		...
		m: function mount(target, anchor) {
			...
			insertNode(dl, target, anchor);
			appendNode(dd, dl);
			appendNode(dt, dl);
			appendNode(text_3, dt);
			appendNode(text_4, dt);
			appendNode(span, dt);
			appendNode(dd_1, dl);
			appendNode(dt_1, dl);
			appendNode(text_8, dt_1);
			appendNode(text_9, dt_1);
			appendNode(span_1, dt_1);
			appendNode(dd_2, dl);
			appendNode(dt_2, dl);
			appendNode(text_13, dt_2);
			appendNode(text_14, dt_2);
			appendNode(span_2, dt_2);
		},

		p: function update(changed, state) {
			if (changed.simple) {
				text_3.data = state.simple;
			}

			if (changed.complex) {
				text_8.data = state.complex;
			}

			if (changed.readable) {
				text_13.data = state.readable;
			}
		},
